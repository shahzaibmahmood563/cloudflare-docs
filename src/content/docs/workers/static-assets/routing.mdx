...
You may wish to run code before assets are served. This is often the case when implementing authentication, logging, personalization, internationalization, or other similar functions. [`serve_directly`](/workers/static-assets/binding/#serve_directly) is a configuration option available in `wrangler.toml` which controls this behavior. When disabled, `serve_directly = false` will invoke your Worker's code, regardless of any assets that would have otherwise matched.
